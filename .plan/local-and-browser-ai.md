...
3. **Enforced by tests:** `mise run test` fails when `.claude/skills/` differs from
   ...
   gitignored and regenerated by `gui:build`, so there is no stale copy to
   check for.
...
- **Still open, re-checked 2026-09-16:**
  - The mise gsx pin and `cmd/gui/go.mod`'s tool pin are two numbers for one
    ...
    `gui:*` builds with `go tool gsx`, so the packslip binary is only what
...
B3 is `gui:deploy`: the same wasm live as `grok-oauth-proxy-gui`, waited for
