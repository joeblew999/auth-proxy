...
command (`go run ./cmd/rel`, a mise task, `go install`):

```bash
rel snapshot
# build release artifacts locally without publishing

rel packslip --bin mytool --resource 'skill/mytool=repo:skills/mytool'
...
rel publish --bin mytool --resource 'skill/mytool=repo:skills/mytool' --version v1.2.0
