// Command rel publishes a GitHub Release fully locally: goreleaser builds the
// artifacts, packslip signs the manifest, gh uploads everything. No workflow.
//
// A repo needs two things to adopt it: a .goreleaser.yml, and a way to run
// this command. Nothing else here is specific to any one repo -- no mise, no
// task names, no layout. The binary name and the skill resource come from
// flags, the repo slug from the origin remote. Run it from the repo root:
//
//	rel snapshot                     build artifacts locally without publishing
//	rel packslip --bin X --resource R build the manifest for the snapshot artifacts and verify it
//	rel publish  --bin X --resource R [--version vX.Y.Z]
//	                                     tag, build, sign, upload (CI uses GITHUB_REF_NAME)
//
// A --resource flag is repeatable: --resource 'skill/foo=repo:skills/foo'.
// In CI, GITHUB_REF_NAME and GITHUB_SHA supply the tag and commit; locally,
// --version supplies the tag and HEAD supplies the commit.
package main
...
const usage = `rel: publish a GitHub Release fully locally (goreleaser + packslip + gh).

  rel snapshot
      build release artifacts locally without publishing
  rel packslip --bin <name> [--resource <spec>]...
      build the packslip manifest for the snapshot artifacts and verify it
  rel publish --bin <name> [--resource <spec>]... [--version <vX.Y.Z>]
...
func main() {
	...
	switch args[0] {
	case "snapshot":
		requireNoArgs(args[1:])
		err = Snapshot()
	case "packslip":
		cfg, rest := parseFlags(args[1:])
		requireNoArgs(rest)
		err = Packslip(cfg)
	case "publish":
		cfg, rest := parseFlags(args[1:])
...
func parseFlags(args []string) (config, []string) {
	...
	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--bin":
			cfg.bin = next(args, &i, "--bin")
		case "--resource":
			cfg.resources = append(cfg.resources, next(args, &i, "--resource"))
		case "--version":
			cfg.version = next(args, &i, "--version")
		default:
...
func next(args []string, i *int, flag string) string {
	if *i+1 >= len(args) {
...
func requireNoArgs(args []string) {
	if len(args) > 0 {
		fail(append([]string{"unexpected"}, args...))
	}
}

func requireBin(cfg config) {
	if cfg.bin == "" {
