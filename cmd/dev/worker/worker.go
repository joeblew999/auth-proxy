...
// Run dispatches a Worker command by verb. Args are everything after it.
func Run(verb string, args []string, stdout, stderr io.Writer) error {
	...
	case "wait":
		...
		if err := fs.Parse(args); err != nil {
			return &UsageError{"wait: " + err.Error()}
		}
		if fs.NArg() != 1 {
			return &UsageError{"wait needs exactly one URL"}
		}
		return Wait(stdout, fs.Arg(0), *timeout)
	}
	return &UsageError{fmt.Sprintf("unknown command %q", verb)}
}
