...
and which one a session loads is luck. `skillpin` makes `skills.toml` the only
thing that decides, and proves it.

## Adopting it

Two things. A `skills.toml` in the repo root:
...
# out and skillpin does not manage that setting at all -- a repo never loses
...
```
go install github.com/joeblew999/grok-oauth-proxy/cmd/skillpin@latest
```

That is all. No task runner is required, nothing is added to your Go module, and
`skillpin` reads no file it is not pointed at. With mise:

```toml
[tools]
"go:github.com/joeblew999/grok-oauth-proxy/cmd/skillpin" = "latest"
```
...
| | |
|---|---|
| `skillpin sync` | write `.claude/skills` from the pins, and the `.claude/settings.json` keys `[claude]` implies |
| `skillpin check` | fail when either has drifted. Put this in CI and in your test task |
| `skillpin verify` | ask a fresh Claude Code session which skills it *actually* sees |
| `skillpin bump [source]` | move github pins to upstream HEAD |
